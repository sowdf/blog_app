var fonts = {
    ...
    logout: function(parent, size, color) {
        var getSize;
        var getColor;
        !size ? getSize = 1 : getSize = size;
        !color ? getColor = "#000000" : getColor = color;
        var can = function() {
            ...
            div.style.position = 'absolute';
            setCanvas.setAttribute('class', 'ss-edit');
            setCanvas.setAttribute('width', 200 * getSize);
            setCanvas.setAttribute('height', 200 * getSize);
            return setCanvas;
        };
        var canvas = can();
        var top = parent.clientHeight / 2 - canvas.parentNode.clientHeight / 2;
        ...
        if (canvas.getContext) {
            var thisFont = canvas.getContext('2d');
            function defaultFont() {
                thisFont.translate(-50 * getSize, 30 * getSize);
                for (var i = 48; i < 50; i = i + 0.1) {
                    thisFont.beginPath();
                    thisFont.arc(150 * getSize, 75 * getSize, i * getSize, Math.PI * 240 / 180, Math.PI * 480 / 180, false);
                    thisFont.strokeStyle = getColor;
                    thisFont.stroke();
                    thisFont.save();
                }
                thisFont.beginPath();
                thisFont.moveTo(160 * getSize, 65 * getSize);
                thisFont.lineTo(160 * getSize, 85 * getSize);
                thisFont.lineTo(120 * getSize, 85 * getSize);
                thisFont.lineTo(126 * getSize, 100 * getSize);
                thisFont.lineTo(100 * getSize, 75 * getSize);
                thisFont.lineTo(126 * getSize, 50 * getSize);
                thisFont.lineTo(120 * getSize, 65 * getSize);
                thisFont.lineTo(160 * getSize, 65 * getSize);
                thisFont.fillStyle = getColor;
                thisFont.fill();
                thisFont.save();
            }
